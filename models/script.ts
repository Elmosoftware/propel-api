...
export class Script extends AuditedEntity implements NativeModel {
    ...
    /**
     * Collection of tags that helps defining the script applicability.
     */
    public tags: Tag[] = [];
    ...
    getModel(): any {
        ...
        //Adding model fields:
        s.add({ name: { type: String, required: true, 
            DESCRIPTION: `Script name.` }});
        s.add({ description: { type: String, required: false, 
            DESCRIPTION: `Brief description of the script that can help others to use it.` }});
        s.add({ isSystem: { type: Boolean, required: true, 
...
Users are able to use System scripts, but not able to alter them.Brief description of the script that can help others to use it.` }});
        s.add({ isTargettingServers: { type: Boolean, required: true, 
            DESCRIPTION: `Boolean value indicating if this script requires a target selection or not.` }});
        s.add({tags: [{ type: Schema.Types.ObjectId, ref: "tag", required: false, 
            DESCRIPTION: `Collection of tags that helps defining the script applicability.` }]});
        s.add({categories: [{ type: Schema.Types.ObjectId, ref: "category", required: false, 
            DESCRIPTION: `Target categories that could be a possible selection for this script. 
If no categories defined, any target could apply.` }]});
        s.add({ readonly: { type: Boolean, required: true, 
            DESCRIPTION: `Indicates if the script is not modifieng target server state in any way.` }});        
        s.add({ code: { type: String, required: true, 
            DESCRIPTION: `Brief description of the script that can help others to use it.` }});
        s.add({parameters: [scriptParameterEmbeddedSchema]});
